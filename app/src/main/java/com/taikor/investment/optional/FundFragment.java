...
public class FundFragment extends BaseFragment {
    ...
    protected void initView(View view, Bundle savedInstanceState) {
        ...
        DividerDecoration divider = new DividerDecoration.Builder(activity)
                ...
                .build();
        rlvFund.addItemDecoration(divider);
        //设置线性布局
        rlvFund.setLayoutManager(new LinearLayoutManager(activity));
        //设置刷新的样式
        rlvFund.setRefreshProgressStyle(ProgressStyle.LineSpinFadeLoader);
        //刷新箭头
        rlvFund.setArrowImageView(R.drawable.ic_pulltorefresh_arrow);
        //加载更多的样式
        rlvFund.setLoadingMoreProgressStyle(ProgressStyle.BallSpinFadeLoader);

        //下拉刷新监听
        rlvFund.setOnRefreshListener(new OnRefreshListener() {
            @Override
        ...
        //加载更多监听
        rlvFund.setOnLoadMoreListener(new OnLoadMoreListener() {
            @Override
            public void onLoadMore() {
                if (mCurrentCount < TOTAL_COUNT) {
                    getData();
                } else {
                    rlvFund.setNoMore(true);
                }
            }
        });

        rlvFund.setHeaderViewColor(R.color.colorAccent, R.color.dark, android.R.color.white);
        //设置底部加载颜色
        rlvFund.setFooterViewColor(R.color.colorAccent, R.color.dark, android.R.color.white);
        //设置底部加载文字提示
        rlvFund.setFooterViewHint("拼命加载中", "已经全部为你呈现了", "网络不给力啊，点击再试一次吧");
        ...
        OkGo.<List<Block>>get(Constant.USER_THEME)
                ...
                .execute(new JsonCallBack<List<Block>>(type) {
                    ...
                    public void onSuccess(Response<List<Block>> response) {
                        ...
                        rlvFund.refreshComplete(REQUEST_COUNT);
