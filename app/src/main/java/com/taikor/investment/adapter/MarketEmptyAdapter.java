...
public class MarketEmptyAdapter extends ListBaseAdapter<MarketAdvice> {

    private Context context;

    public MarketEmptyAdapter(Context context) {
        super(context);
    ...
    public int getLayoutId() {
        return R.layout.item_market_empty;
    }

    @Override
    public void onBindItemHolder(SuperViewHolder holder, final int position) {
        CircleImageView photo = holder.getView(R.id.iv_empty_photo);
        Glide.with(context).load(mDataList.get(position).getImageUrl()).into(photo);

        TextView name = holder.getView(R.id.tv_empty_name);
        name.setText(mDataList.get(position).getPersonName());
        TextView date = holder.getView(R.id.tv_empty_date);
        ...
        date.setText(t);
        TextView title = holder.getView(R.id.tv_empty_title);
