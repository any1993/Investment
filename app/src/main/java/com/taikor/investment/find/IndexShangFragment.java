...
public class IndexShangFragment extends BaseFragment {

    @BindView(R.id.tv_stock_shang)
    TextView tvStockType;
    @BindView(R.id.tv_shang_price)
    TextView tvStockSettlement;
    @BindView(R.id.tv_shang)
    TextView tvUpDown;
    @BindView(R.id.tv_shang_asset)
    TextView tvAsset;
    @BindView(R.id.shang_chart)
    LineChart lineChart;
    ...
    private void getEmotionIndex() {
        ...
        OkGo.<List<Emotion>>get(Constant.STOCK_SHANG)
                .tag(activity)
                .headers("Authorization", token)
                .params("type", 0)
                .params("count", 5)
    ...
    private void getAssetData() {
        ...
        OkGo.<List<Asset>>get(Constant.ASSET)
                .tag(activity)
                .headers("Authorization", token)
                .params("type", 0)
                .params("skipHour", 1)
                ...
                .execute(new JsonCallBack<List<Asset>>(type) {
                    ...
                    public void onSuccess(Response<List<Asset>> response) {
                        ...
                        if (mainForce > 0) {
                            ...
                            tvAsset.setText("上证指数 流入 " + CommonUtils.setDoubleTwo(mainForce / 10000) + "亿");
                        } else if (mainForce < 0) {
                            tvAsset.setTextColor(ContextCompat.getColor(activity, R.color.down));
                            String s = CommonUtils.setDoubleTwo(mainForce / 10000);
                            tvAsset.setText("上证指数  流出  " + s.substring(1, s.length()) + "亿");
                        }
