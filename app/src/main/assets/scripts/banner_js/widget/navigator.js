    ...
    $.ui.define("navigator", {
        ...
        _setup: function (fullMode) {
            ...
            $el.find('a').each(function (i) {
                defTab === 0 ? $(this).hasClass('cur') && (data.defTab = i) : $(this).removeClass('cur') && $(this).siblings().addClass('Page');    //处理同时defTab和写cur class的情况
            });
        },
        _init: function () {
            var me = this,
        ...
        switchTo: function (index, isDef, e) {
            if ((TopIndex == 0 && loadingStep != 1 && loadingStep != 2) || (TopIndex != 0 && loadingStepMore[TopIndex] != 1 && loadingStepMore[TopIndex] != 2)) {
                var me = this,
                    data = me._data,
                ...
                lastIndex >= 0 && $tabList.eq(lastIndex).removeClass("cur") && $tabList.eq(lastIndex).siblings().addClass('Page');      //修改样式放在跳转后边
                $tabList.eq(index).addClass("cur");
                $tabList.eq(index).siblings().removeClass('Page');
                if ($tabList.eq(index).find("input").val() != undefined) {
                    top_tagclick($tabList.eq(index).find("input").attr("IndexID"), $tabList.eq(index).find("input").val(), $tabList.eq(index).text());
                }
