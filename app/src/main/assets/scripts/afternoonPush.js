...
function bind_indexlist(id) {
    $('#' + id).bind('click', function (event) {
        ...
        AddGoback(localStorage.N_url + '&scrollTop=' + $(window).scrollTop(), 'stock.html?stockId=' + id);
    })
}
//绑定主题的点击
function bind_indexlist1(id) {
    $('#' + id).bind('click', function (event) {
        event.preventDefault();
        event.stopPropagation();
        AddGoback(localStorage.N_url + '&scrollTop=' + $(window).scrollTop(), 'opportunityPage3o1_TopicDetails.html?itemId=' + id);
    })
}
    ...
    Wechat.share({
        message: {
            ...
            media: {
                ...
                webpageUrl: "http://m.boolcj.com/afternoonPush.html?pointTime=" + pointTime,    // webpage
            }
        },
...
function touchstartWebo(event) {
    ...
    var args = {};
    args.url = "http://m.boolcj.com/afternoonPush.html?pointTime=" + pointTime;
    args.title = pointDate + '布尔下午茶';
...
function touchstartSms(event) {
    ...
    window.plugins.socialsharing.shareViaSMS("http://m.boolcj.com/afternoonPush.html?pointTime=" + pointTime, null /* see the note below */, function (msg) {
...
function touchstartCopy(event) {
    event.preventDefault();
    cordova.plugins.clipboard.copy("http://m.boolcj.com/afternoonPush.html?pointTime=" + pointTime, function (msg) {
